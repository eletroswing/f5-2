...
_ref_audio_cache = {}
# Initialize EspeakBackend for PT-BR (or other languages if needed)
...
# Function to preprocess and phonemize text into IPA
def phonemize_text(text, backend=phonemizer_backend):
    text = text.lower()
    text = re.sub(r"\s+", " ", text.strip())
    return backend.phonemize([text])[0]  # Returns a string in IPA
...
# Updated preprocess_ref_audio_text
def preprocess_ref_audio_text(ref_audio_orig, ref_text, show_info=print, device=device):
    ...
    show_info("Phonemizing reference text to IPA...")
...
def infer_process(
    ...
    show_info("Phonemizing generation text to IPA...")
    gen_text_ipa = phonemize_text(gen_text)
    show_info(f"Phonemized generation text: {gen_text_ipa}")

    # Split the input text into batches
    audio, sr = torchaudio.load(ref_audio)
    gen_text_batches = chunk_text(gen_text_ipa)

    for i, batch in enumerate(gen_text_batches):
        print(f"gen_text_batch {i}: {batch}")
